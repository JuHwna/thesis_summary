...
- 다행히도, 커뮤니티에서 그래프 구조 데이터의 모델링을 위한 Graph Neural Networks(GNNs)이 성공하면서 표현 학습 기반의 신경망을 위해 유저-아이템 이분 그래프 구조를 모델링하는 많은 선행 연구가 제안되었음
- 유저-아이템 이분 그래프가 주어졌을 때, 자유 유저 잠재 행렬과 유저 잠재 행렬을 고전적인 잠재 요인 기반 모델, 즉 0차 사용자 및 아이템 임베딩의 수만큼의 자유 사용자 잠재 행렬과 아이템 잠재 행렬을 나타내는 $P^0$와 $Q^0$로 함
- 이 뉴럴 그래프 기반의 모델들은 (l+1)번째 유저(아이템) 임베딩을 l번째 아이템(유저) 임베딩의 집계로 반복적으로 업데이트함. 예를 들어 각 유저의 업데이트된 임베딩 $p^(l+1)_u$는 다음과 같이 계산된다.
  - ![image](https://user-images.githubusercontent.com/49123169/237046419-d886e4a4-986b-4158-8e78-c6ab1359cfc2.png)
  - $q^l_j 는 l번째 층에서 아이템 j의 표현
  - $R_u$는 유저-아이템 이분 그래프에서 유저 u에 연결되는 항목을 나타냄
  - $a^(l+1)_u$는 l번째 층에서 연결된 아이템의 표현을 합산한 값
  - $W^l$은 학습해야하는 임베딩 변환 매트릭스
  - $p()$는 활성화 함수
  - 이후, 각 유저의(아이템의) 최종 임베딩은 각 레이어에서 각 엔티티의 임베딩을 결합한 것으로 볼 수 있음
- 위의 단계는 유저-아이템 이분형 그래프에 임베딩 전파를 포함하는 것으로 볼 수 있음. 미리 정의된 레이어 L을 사용하면 최대 L차 하위 그래프 구조가 유저 및 아이템 임베딩 표현 단계에서 직접 인코딩됨
- 예를 들어, Spectral CF는 CF에 스펙트럼 그래프 컨볼루션을 활용했음. GC-MC와 NGCF는 원래 공간에서 유저-아이템 상호작용의 그래프 컨볼루션을 모델링했으며, 실제로는 더 효과적이고 효율적임 
- 꽤 최근에, 연구자들은 뉴럴 그래프 기반 CF 모델이 유저 또는 아이템 특징을 포함하지 않기 때문에 기존 GNN 모델들과 다르지 않다고 주장했음.
- GNN에서 임베딩 변환와 비선형 활성함수 같은 복잡한 단계를 직접 차용하는 것은 좋은 선택이 아닐 수 있음. 불필요한 딥러닝 연산을 제거하는 LR-GCCF, LightGCN을 포함한 간소화된 신경 그래프 CF 모델이 제안되었음
- 이 간소화된 신경 그래프 기반의 모델들은 신중하게 선택한 활성화 함수 없이도 실전에서 최고의 성능을 보임

### 2.2 상호작용 모델링
- $p_u$와 $q_i$는 표현 모델에서 유저 u와 아이템 i의 학습된 임베딩을 나타내는데, 이 요소는 그들의 표현을 기반으로 타겟 아이템에 대한 유저의 선호도를 예측하는 상호 함수 모델링을 목표로 함
- 다음에서는 우리는 학습된 임베딩을 기반으로 $\hatr_{ui}$로 표시되는 유저의 예측된 선호도를 모델링하는 방법을 설명함. 설명의 예시로 테이블 2를 보면, 우리는 상호작용 모델링을 (전통적인 내적 곱 기반의 접근, 거리 기반의 모델링, 뉴럴 네트워크 기반의 접근) 세 가지 주요한 카테고리로 요약한다.
- ![image](https://github.com/JuHwna/thesis_summary/assets/49123169/fa2f7901-c21a-4346-a3ea-6cc0a13eaace)
- 대부분의 이전 추천 모델들은 유저와 아이템 쌍 점수를 추정하기 위해 유저 임베딩과 아이템 임베딩 사이의 내적에 의존했다. $\hat{\gamma_{ui}}=p^T_u q_i=\sum{f=1}^dp_{uf}q_{if}$
- 이전 추천 모델의 큰 성공과 간편성에도 불구하고 이전 연구 결과에 따르면 단순히 내적을 수행하는데 두 가지 한계를 가지고 있음
  - 첫 번째, 삼각형 부등식이 위반함. 그러므로 내적은 유저와 과거 아이템의 표현이 유사하도록 권장할 뿐, 유저-유저와 아이템-아이템 관계 간의 유사성 전파에 대한 보장이 부족함 
  - 두 번째, 그것은 선형 상호작용 모델이고 유저와 아이템 사이의 복잡한 관계를 발견하는데 실패할 수 있음

#### 2.2.1 거리 기반 메트릭(지표)
- 첫 번째 이슈를 해결하기 위해서 일련의 연구인 CML,TransRec,LRML에서 번형 원리의 아이디어를 빌려 거리 측정값을 상호작용 함수로 사용하는 연구를 진행중임. 
