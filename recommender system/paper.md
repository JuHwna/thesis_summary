...
- 하나의 대표적인 연구는 유저 인식 협업 필터링 -> ACF(Attentive Collaborative Filtering)으로, 각 상호작용된 아이템에 유저 인식 가중치를 부여하여 유저 표현에 대한 중요성을 나타냄. 
  - ![image](https://user-images.githubusercontent.com/49123169/236977269-655124b3-e1db-488a-9357-85a8b94d738d.png)
  - $p_u$ 는 유저 u의 id 임베딩이고 $R_u$ 는 유저가 상호작용한 항목을 나타냄. $\alpha (u,j)는 다음과 같이 정의된 세심한 가중치임
    - ![image](https://user-images.githubusercontent.com/49123169/236977584-211796e6-1728-461d-a815-a5065f76655a.png)
    - $F()는 MLP이거나 단순히 내적으로 구현할 수 있는 함수
- 실용적으로 히스토리컬 아이템의 영향은 타겟 아이템에 의존할 수 있다. 예를 들어 바지의 구입이 이전 셔츠 구입과 매우 연관이 있는 것처럼 폰 케이스의 구입은 이전 휴대폰 구입에 매우 연관되어 있다. 따라서 다양한 타겟 아이템에 대한 예측을 고려할 때 동적 유저의 표현을 사용하는 것이 유용할 수 있음. 이를 위해 Neural Attentive Item Similarity(NAIS)은 주의 메커니즘을 대상 항목 인식으로 수정한다.
  - ![image](https://user-images.githubusercontent.com/49123169/236991216-4743adc6-0112-4f29-a6fb-b5e84b0b5da3.png)
  - 타겟 아이템 i에 대한 유저의 선호도를 예측할 때, $\alpha (i,j)$는 과거 아이템 j가 유저 표현에 기여하는 정도를 나타냄
  - $\beta$ 는 다양한 길이의 상호 히스토리를 평활화하기 위한 0과 1 사이의 하이퍼 파라미터임
- 비슷한 주의 메카니즘은 상호 히스토리로부터 표현 학습에도 채택되었음. 예를 들어 Deep Item-based CR model(DeepICF), Deep Interest Network(DIN)
- 따라서 상호 히스토리(상호작용기록)는 단일 사용자 id보다 좀더 많은 정보를 포함하고 있고 표현 학습에 적합한 선택임.

#### 2.1.2. 표현 학습 기반의 오토인코더
- 오토인코더 기반 모델은 더 나은 표현 학습을 위해 입력을 재구성한다는 아이디어를 활용하여 불완전한 유저-아이템 행렬을 입력으로 받아 인코더를 통해 각 인스턴스의 숨겨진 표현을 학습하고, 나아가 숨겨진 표현을 기반으로 입력을 재구성하는 디코더 부분을 통해 학습을 진행함.
- 각 유저의 과거 기록을 입력으로 처리함으로써 오토인코더 기반 모델은 복잡한 인코더 신경망으로 각 유저의 잠재적 표현을 학습하고 학습된 유저 표현을 디코더 네트워크에 공급하여 각 유저의 예측된 선호도를 출력함. 다른 접근 방식은 모든 유저로부터 각 아이템의 평점 레코드를 입력으로 가져와서 아이템의 잠재적 표현을 학습하여 모든 유저로부터 각 아이템의 예측된 선호도를 재구성하는 것임
- 오토인코더의 개발과 마찬가지로 오토인코더 기반의 모델의 확장은 두 가지 범주로 분류할 수 있음
  - 첫 번째 범주는 오토인코더 변현을 활용하고 노이즈 제거 오토인코더(injected denoising autoencoder), 변형 오토인코더(variational autoencoder)를 CF에 주입했음. 이 모델들은 각각 유저 또는 아이템 인코더들을 학습하기 위해 복잡한 딥러닝 기술들을 사용하는 것으로 볼 수 있음.
  - 두 번째 범주는 오토인코더에서 유저와 아이템의 이중성을 활용하고 두 개의 병렬 인코더를 설계하여 유저와 아이템의 표현을 학습한 다음, 내적 곱을 사용하여 아이템에 대한 유저의 선호도를 모델링했음. 오토인코더 기반의 접근 방식은 과거 행동을 집계하기 위해 심층 신경망(deep neural networks)을 채택하기 때문에 과거 행동 주의 기반 모델의 확장으로 분류할 수도 있음. 따라서 간단하게 설명하기 위해 오토인코더 기반 모델에 대해서만 간략하게 소개했으며 구체적인 기술적 세부 사항을 반복하지 않았음

#### 2.1.3. 그래프 기반 표현 학습
- CF 효과는 다수 유저의 상호 과거 이력이 반영되었다. 따라서 집단 상호 과거 이력을 사용하면 표현 품질을 개선할 수 있음.
- 유저 아이템 상호작용 그래프의 관점에서 개별 상호작용 과거 기록은 유저의 1차 연결과 동일함. 따라서 자연스럽게 유저-아이템 그래프 구조에서 고차 연결성을 마이닝하는 것으로 확장할 수 있음. 예를 들어 유저의 2차 연결은 동일한 아이템과 함께 상호작용한 유사한 유저로 구성됨. 
- 그럼에도 불구하고, 컴퓨니티에서 그래프 구조 데이터의 모델링을 위한 Graph Neural Networks(GNNs)의 성공과 함께 대다수의 우선적인 연구들은 
