...
- B. Few-shot-CoT
  - 문제 해결 과정을 단계별로 설명(Few-shot과 유사) => 각 단계를 명시적으로 보여줌, 중간 추론 과정을 공개
- C. Zero-shot
  - 사전 학습된 지식만으로 문제 해결 => 예시 없이 바로 답 도출, 최소한의 맥락으로 문제 해결
- D. Zero-shot-CoT
  - 사전 예시 없이 문제 해결 "Let's think step by step"를 프롬프트에 추가 => 단계별 추론 과정 포함, 각 단계를 논리적으로 설명

#### 답변하기 전에 생각하기로 미세 조정
- Deepseek R1이나 OpenAI의 o1 모델
  - 특정 사고 단계를 항상 포함하도록 훈련됨
    - 특수 토큰 <think>와 </think> 사이
  - 모델이 수천 개의 예시를 분석
  - 특정 사고 단계를 생성하도록 학습하는 훈련 방법

### 3. 행동 : 에이전트와 플랫폼과의 상호 작용
#### 에이전트가 플랫폼과 상호작용하기 위해 취하는 구체적인 단계
- 예시 : 고객 지원 에이전트
  - 정보 검색 : 고객 데이터 검색
  - 장치 제어 : 가이드 문서 제공
  - 전달 : 고객의 문제점을 담당자에게 전달
 
#### 행동 방식에 따른 에이전트 행동 유형

|에이전트 유형|설명|
|JSON 에이전트|취할 행동이 JSON 형식으로 지정|
|코드 에이전트|에이전트가 외부에서 해석되는 코드 블록을 작성|
|함수 호출 에이전트|각 행동에 대해 새 메시지를 생성하도록 파인 튜닝된 JSON 에이전트의 하위 카테고리|

#### 목적에 따른 에이전트 행동 유형
|행동 유형|설명|
|정보 수집|웹 검색, 데이터 베이스 쿼리, 문서 검색 수행|
|툴 사용|API호출, 계산 실행, 코드 실행|
|상호작용|디지털 인터페이스 조작 또는 물리적 장치 제어|
|통신|채팅을 통한 사용자와의 상호작용 또는 다른 에이전트와의 협업|

#### 행동 종료 및 출력 제어
- 행동이 완료됨녀 새 토큰 생성을 중지한느 기능
  - JSON, 코드, 함수 호출 등 모든 형식의 에이전트에 적용
  - 의도하지 않은 출력을 방지
  - 에이전트의 응답을 명확하고 정확하게 유지
  - LLM은 텍스트만 처리
    - 취하고자 하는 행동과 사용하고자 한느 툴에 제공할 매개변수를 설명

#### 중지 및 파싱 접근법
- 구조화된 형식
  - 명확하고 설정한 형식(JSON 또는 코드)으로 행동 출력
- 추가 생성 중단
  - 행동이 완료되면 추가 토큰 생성 중지 -> 잘못된 출력 방지
- 출력 파싱
  - 외부 파서가 포맷된 행동을 읽고 호출할 툴을 결정하고 필요한 매개변수 호출

![image](https://github.com/user-attachments/assets/f27c1b0f-ed7d-49c3-bf0a-3ec0c03e6981)

#### 코드 에이전트의 장점
- 표현력
  - 복잡한 로직을 자연스럽게 표현 가능한 코드로 표현 -> JSON보다 더 큰 유연성 제공
- 모듈성과 재사용성
  - 다른 행동이나 작업에서 재사용할 수 있는 함수와 모듈로 생성
- 디버깅 용이성
  - 잘 정의된 프로그래밍 구문 -> 코드 오류를 더 쉽게 감지하고 수정 가능
- 직접 통합
  - 외부 라이브러리 및 API와 직접 통합 가능 -> 데이터 처리 또는 실시간 의사 결정과 같은 복잡한 작업 가능

### 4. 관찰 : 피드백 통합을 통한 성찰과 적

#### 에이전트가 자신의 행동의 결과를 인식하는 방식
- 피드백 수집 : 행동의 성공 여부에 대한 데이터 확인
- 결과 추가 : 새로운 정보를 기존 컨텍스트에 통합하여 메모리 업데이트
- 전략 조정 : 업데이트된 컨텍스트로 후속 사고와 행동을 정제
- 지속적인 피드백 입력을 통해 에이전트가 일관된 목표를 유지할 수 있도록 보장
...
|환경 데이터|센서 판독, 시스템 메트릭, 리소스 사용량|
|응답 분석|API 응답, 쿼리 결과, 계산 출력|
|시간 기반 이벤트|마감 시간 도달, 예약된 작업 완료|

## [Bonus] 함수 호출을 위한 모델 파인 튜닝
### 1. 함수 호출
- 핵심 아이디어
  - 프롬프트 기반 접근 방식 => 함수 호출을 통해 모델이 훈련 단계에서 행동하고 관찰

#### 함수 호출이란
- LLM이 외부 도구나 서비스와 상호작용하여 작업을 수행하는 방법
  - GPT-4에서 처음 소개 -> 이후 다른 모델에서도 함수 호출 사용
  - 함수 호출은 모델에게 다양한 작업과 서비스를 직접 실행할 수 있는 능력 제공
  - 모델이 함수 호출 능력을 학습하며 다른 에이전트 기술보다 프롬프팅에 덜 의존적
  - 함수 호출을 통해 에이전트가 도구를 사용하도록 미세 조정(Fine-tuning)됨

#### (기존) 프롬프트 기반 접근 방식
1. 사고 : 목표를 달성하기 위해 어떤 단계와 툴이 필요한가
2. 행동
  - 필요한 툴에 정확한 지시사항과 데이터를 전달
  - 작업 실행 준비
3. 관찰 : 툴이 수행한 작업의 결과를 확인하고 분석

<img width="675" height="445" alt="image" src="https://github.com/user-attachments/assets/dbbd1162-8b42-4bad-af6c-c8a016238304" />

#### 함수 호출을 통한 상호작용
- 기존 대화 모델은 user와 assistant 역할만 인식
  - 함수 호출을 도입하면서 새로운 방식의 상호작용 가능
- 새로운 역할 도입
  - 행동을 위한 새로운 역할
  - 관찰을 위한 새로운 역할

<img width="670" height="533" alt="image" src="https://github.com/user-attachments/assets/df3d339c-fcdc-4772-a207-fada75196692" />

#### 특수 토큰

<img width="1220" height="454" alt="image" src="https://github.com/user-attachments/assets/3f8977d0-34fe-4cd8-9405-810a7447986c" />


## 2. LoRA
### 함수 호출을 위한 모델 훈련 과정
- 모델 사전 훈련
  - 대량의 데이터를 통해 모델 사전 훈련
  - 다음 토큰 예측만 가능
  - 예시 : 기본 Gemma 2(gemma-2-2b)
- 미세 조정
  - 모델에 대화 능력 추가
    - 사용자의 지시 이해
    - 대화 맥락 이해
    - 구조화된 답변 생성
  - 예시 : 미세 조정된 Gemma 2(gemma-2-2b-it)
- 원칙 정립
  - 제작자가 원하는 바람직한 행동을 하도록 훈련
  - 예시 : 사용자에게 항상 친절한 챗봇 모델

### Low-Rank Adaptation of Large Language Models
- 학습 가능한 매개변수의 수를 감소시키는 훈련 기법
- 모델의 대부분을 동결하고 새로 추가된 어댑터 가중치만 업데이트
- 훈련 과정이 매우 빠르고 효율적임
- 크기가 작은 가중치 생성 -> 저장 및 공유 용이
- 추론 시 입력을 어댑터와 기본 모델에 전달 또는 가중치 병합 가능
  - 추가 대기 시간 오버헤드가 없음
- 리소스 제약을 최소화하면서 LLM을 특정 작업에 최적화 시킬 수 있음

<img width="680" height="353" alt="image" src="https://github.com/user-attachments/assets/76b9614e-fbd0-45aa-a58c-b0f6dc894f4c" />
