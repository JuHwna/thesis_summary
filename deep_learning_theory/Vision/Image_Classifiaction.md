...
## 이미지 분류 모델 이해를 위한 사전 지식
### Convolutional Layer
- 입력 데이터로부터 특징을 추출하는 역할
- 구성
  - Filter : 특징을 추출하는 기능
  - Activation 함수 : 이 필터의 값을 비선형 값으로 바꾸어 줌
![image](https://github.com/JuHwna/thesis_summary/assets/49123169/fbe535c3-499f-47cc-aa38-753d3c6822c7)

- Filter(필터)
  - 개념 : 특징이 데이터에 있는지 없는지 검출해주는 함수
  - 입력받은 데이터에서 그 특성을 가지고 있으면 결과 값이 큰 값이 나오고 특성을 가지고 있지 않으면 결과 값이 0에 가까운 값이 나오게 됨
    - 데이터가 그 특성을 가지고 있는지 없는지 여부를 알 수 있게 해줌
  - 다중 필터 적용 시
    - 입력 값에는 여러 가지 특징이 있기 때문에 하나의 필터가 아닌 여러 개의 다중 필터를 같이 적용하게 됨
    - 각기 다른 특징을 추출하는 필터를 조합하여 네트워크에 적용하면 원본 데이터가 어떤 형태의 특징을 가지고 있는지 없는지 판단해낼 수 있음

- Stride
  - 필터를 적용하는 간격
![image](https://github.com/JuHwna/thesis_summary/assets/49123169/453c8a4c-cea2-430c-abb8-b242c0c13b28)
...
- Padding
  - 결과 값이 작아지는 것을 방지하기 위해서 입력값 주위로 0 값을 넣어서 입력 값의 크기를 인위적으로 키워서 결과값이 작아지는 것을 방지하는 기법
  - 원본 데이터에 필터를 적용한 내용을 보면 필터를 적용한 후의 결과값은 필터 적용전보다 작아짐
    - CNN 네트워크는 하나의 필터 레이어가 아니라 여러 단계에 걸쳐서 계속 필터를 연속적으로 적용하여 특징을 추출하는 것을 최적화해 나감
    - 필터 적용 후 결과 값이 작아지게 되면 처음에 비해서 특징이 많이 유실될 수 있음
      - 필터를 거쳐감에 따라서 특징이 유실되는 것을 기대하면 좋겠지만 아직까지 충분히 특징이 추출되기 전에 결과 값이 작아지면 특징이 유실됨
  - 해당 문제를 방지하기 위해 padding 기법 사용
  - padding을 사용하는 이유
    - 결과 값을 작아지는 것을 막아서 특징이 유실되는 것을 막음
    - 오버피팅 방지
      - 원본 데이터에 0값을 넣어서 원래의 특징을 희석시킴
      - 이것을 기반으로 머신러닝 모델이 트레이닝 값에만 정확하게 맞아 들어가는 오버피팅 현상 방지
