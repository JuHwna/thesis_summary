...
- 기울기 사라짐(Gradient Vanishing) 문제와 활성화 함수(Activation Function)
  - 여러 개의 layer가 쌓였을 경우 발생하는 대표적인 문제 : 기울기 사라짐
  - 뉴럴넷 핛ㅂ에서는 오차 역전파 방법을 사용함
  - 오차 역전파는 필연적으로 과거 사용된 활성화 함수의 도함수가 여러번 곱해지게 됨
    - 만약 시그모이드와 같은 활성화 함수를 사용했다면 필연적으로 기울기 사라짐 문제가 발생함
    - 이러한 현상은 활성화 함수의 도함수가 여러 번 곱해지는 input layer 근방에서 더 크게 나타남
  - 해결 방법
    - 활성화 함수를 변경해 어느 정도 막을 수 있음
      - 일반적으로 hidden layer에서는 활성화 함수로 ReLU 계열을 사용하고 마지막 Output layer에서만 sigmoid와 같이 목적에 맞는 활성화 함수를 사용함

- 오버피팅(Overfitting)과 정규화(Regularization)
  - 언더피팅(Underfitting), 오버피팅(Overfitting)
    - 지도 학습 딥러닝 모델의 목표 : 주어진 데이터를 이용해 데이터를 잘 설명하는 모델을 만들고 만들어진 모델을 이용해서 학습하지 않은 데이터에서도 예측력이 우수한 모델을 만드는 것
      - Gradient descent를 이용해서 예측값과 현상(label)간의 오차를 최대한 줄이는 방법을 이용하여 학습함
    - 언더피팅 : 딥러닝 학습에 사용한 신경망 구조가 너무 단순해서 Training 데이터와 Test 데이터 모두를 설명하고 예측하지 못하는 경우
      - 해결 방법
        - 주어진 데이터를 더 잘 설명하는 적당한 모델을 선택/개발하는 방법
        - 학습을 더 많이 하는 방법
    - 오버피팅 : 딥러닝 학습에 복잡한 모델을 사용하면 Training 데이터에 대해서는 정확히 설명할 수 있으나 Test 데이터의 예측력이 매우 떨어지는 경우
      - 해결 방법
        - 기존 학습 알고리즘에 여러 가지 정규화 방법을 추가해서 해결함
        - (1) 얼리 스탑(Early stopping)
          - 오버피팅을 막는 가장 손쉬운 바업
          - 트레이닝 세트에 가중치가 최적화되기 이전에 학습을 조기종료시키는 방법
          - 가중치 업데이트에 테스트 세트를 포함시키지 않고 에러 모니터링만 함
            - 테스트셋의 학습이 진행될수록 사용자가 지정한 횟수 동안 로스가 상승한다면 오버핏이 나타난다고 간주하고 학습을 중지함
        - (2) Weight decay
        ...
        - (3) Dropout
          - 랜덤 포레스트의 앙상블과 매우 유사한 방법
            - 랜덤 포레스트 : 서로 다른 Feature를 랜덤으로 선택하고 만들어진 Decision Tree들의 앙상블로 결과값을 예측하는 모델
          - 뉴럴넷의 Dropout의 경우, 미니 배치마다 히든 레이어에서 임의의 노드들을 랜덤으로 선택해서 학습을 진행함
            - 남겨진 퍼셉트론과 연결된 가중치는 drop 안할 때보다 더 강한 가중치 값을 가지게 됨
            - 실제 테스트 시에는 drop하지 않고 모든 가중치에 (1-drop_rate)%를 곱해 전체 가중치를 한번 스케일링하여 Prediction함
