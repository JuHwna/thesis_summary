...
- ex)타겟 시퀀스 가운데 went를 맞힐 차례면 인코더 입력은 소스 시퀀스 전체, 디코더 입력은 s I
  - 여기에서 특이한 점
    - 학습 중의 디코더 입력과 학습을 마친 후 모델을 실제 기계 번역에 사용할 때(인퍼런스)의 디코더 입력이 다르다는 점
    - 학습 과정에서는 디코더 입력에 맞혀야할 단어 이전의 정답 타겟 시퀀스를 넣어줌
    - 학습 종료 후 인퍼런스 때는 현재 디코더 입력에 직전 디코딩 결과를 사용함

## (4) 트랜스포머 블록
![image](https://github.com/JuHwna/thesis_summary/assets/49123169/2c369124-55e4-434d-8c73-8d22cd319d86)

- 트랜스포머의 인코더 가운데 반복되는 요소를 떼어내 다시 나타낸 것
  - 이런 구조를 블록(block) 또는 레이어(layer)
  - 트랜스포머의 인코더는 이같은 블록을 수십 개 쌓아서 구성함
- 인코더 블록의 세 가지 요소
   1. Self Attention
      - 1. 모델 입력과 출력
         - ![image](https://github.com/JuHwna/thesis_summary/assets/49123169/aafe75a9-3fd8-49e0-a1e7-c5bb39d5b6de)
         - 입력층 : 그림과 같이 모델 입력을 만드는 계층
         - 인코더 입력은 소스 시퀀스의 입력 임베딩(input embedding)에 위치 정보를 더해서 만듦
         - 
...
# SVD(특이값 분해)
- SVD(Singular Value Decomposition) : 직교하는 벡터집합에 대하여 선형 변환 후에 그 크기는 변하지만 여전히 직교할 수 있게 되는 그 직교 집합은 무엇인가? 그리고 선형 변환 후의 결과는 무엇인가?
  - 복소수 공간에 대하여 정의하는 것이 일반적이지만 본 페이지에서는 실수 벡터 공간에 한정하여 작성
